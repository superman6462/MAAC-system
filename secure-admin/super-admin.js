...
============================================================ */
const GAS_WEBAPP_URL = 'https://script.google.com/macros/s/AKfycbxsRG9NrvH3Wt0FgIsAw0u5qLqmoVi310KtWfAc1QqYErJkAlis0hApcFbtAu2NdtASpw/exec';
...
const TABS = {
  ...
  async hero(area) {
    ...
    area.innerHTML = `
      ...
      <div class="card" style="margin-bottom:18px;">
        <div class="card-header"><span class="card-title">Add Hero Image</span></div>
        <div class="form-grid form-grid-2">
          <div class="form-group" style="grid-column:1/-1;">
            <label class="form-label">Image Source</label>
            <div style="display:flex;gap:8px;margin-bottom:8px;">
              <button type="button" class="btn btn-ghost btn-sm" id="hTabFile" onclick="switchHeroTab('file')" style="background:rgba(201,168,76,0.12);color:var(--gold);"><i class="fas fa-upload"></i> Upload File</button>
              <button type="button" class="btn btn-ghost btn-sm" id="hTabUrl"  onclick="switchHeroTab('url')"><i class="fas fa-link"></i> Image URL</button>
            </div>
            <div id="hFileWrap"><input class="form-input" type="file" id="hFile" accept="image/*"></div>
            <div id="hUrlWrap" style="display:none;"><input class="form-input" id="hUrl" placeholder="https://example.com/image.jpg"></div>
          </div>
          <div style="align-self:end;"><button class="btn btn-primary" id="addHeroImgBtn"><i class="fas fa-plus"></i> Add Image</button></div>
        </div>
      </div>

      <div class="card">
        <div class="card-header"><span class="card-title">Hero Images</span></div>
        <div id="heroImgGrid" style="display:grid;grid-template-columns:repeat(auto-fill,minmax(160px,1fr));gap:12px;"></div>
      </div>`;
    ...
    window.switchHeroTab = (tab) => {
      ...
      const ub = document.getElementById('hTabUrl');
      if (tab === 'file') {
    ...
    function renderHeroImgGrid(list) {
      const gridEl = document.getElementById('heroImgGrid');
      if (!gridEl) return;
      if (!list.length) {
        gridEl.innerHTML = '<div class="empty-state" style="grid-column:1/-1"><i class="fas fa-images"></i><p>No hero images yet</p></div>';
        return;
      }
      gridEl.innerHTML = '';
      list.forEach((img, idx) => {
        ...
        div.dataset.idx = idx;
        div.innerHTML = `
          <img src="${img.url}" style="width:100%;height:130px;object-fit:cover;display:block;" loading="lazy"
               onerror="this.src='https://placehold.co/160x130/152036/c9a84c?text=Error'">
          <button class="hero-img-delete-btn" data-idx="${idx}"
  ...
  async teachers(area) {
    ...
    window.addTeacher = async () => {
      ...
      if (isUrlMode) {
        ...
        if (file) {
          try { const result = await uploadViaGAS(file, 'Teachers'); photo = result.url; }
  ...
  async gallery(area) {
    ...
    window.addGalleryImage = async () => {
      ...
      if (isUrlMode) {
        ...
        try { const result = await uploadViaGAS(file, 'Gallery'); url = result.url; fileId = result.fileId; }
        catch(e) { showToast('Upload error: ' + e.message, 'error'); return; }
      }
      try {
        await db.collection('galleries').add({ url, fileId, caption: document.getElementById('gCap').value, uploadedAt: firebase.firestore.FieldValue.serverTimestamp() });
