/* ============================================================
   MAAC Super Admin — super-admin.js
   Drive operations now route through Google Apps Script (GAS)
   instead of direct GIS OAuth calls.

   ⚙️  SETUP REQUIRED:
   1. Create a new GAS project at script.google.com
   2. Paste the MAAC GAS script (same logic as your other project)
      but set FOLDER_ID to your MAAC Drive folder ID
   3. Deploy → Web app → Execute as: Me → Access: Anyone
   4. Copy the deployment URL and paste it below as GAS_WEBAPP_URL
============================================================ */
...
/* ============================================================
   GOOGLE DRIVE VIA GAS  (replaces GIS OAuth approach)
   — No popup, no OAuth token, no client-side Drive API calls.
   — All Drive operations go through your GAS web app.
============================================================ */
const GAS_WEBAPP_URL = 'https://script.google.com/macros/s/AKfycbzJ30o2a5MkrRC4OTSpBPT-bLhMnxJRjfUauaJxOG8Vkl8IhCZXMY4x-4Y-_K174nl7oQ/exec';
...
/**
 * Upload a file to Google Drive via GAS web app.
 * Returns { url, fileId } on success.
 *
 * @param {File}   file       - The File object selected by user
 * @param {string} folderName - Sub-folder name inside the root Drive folder
 */
async function uploadViaGAS(file, folderName = 'Uploads') {
  setDriveStatus('Uploading…', false);
  try {
    ...
    setDriveStatus('Drive error', false);
    throw e;
  }
}

/**
 * Delete a file from Google Drive via GAS web app.
 * Moves the file to trash (recoverable).
 *
 * @param {string} fileId - Google Drive file ID stored in Firestore
 */
async function deleteFileFromDrive(fileId) {
  ...
  try {
    ...
    console.warn('Drive delete error (file may still exist on Drive):', e.message);
    // Don't block Firestore deletion if Drive delete fails
  }
}
...
async function checkAccess(user) {
  ...
  try {
    ...
    document.getElementById('loadingScreen').innerHTML =
      `<div class="loader-error">Error: ${e.message}<br><a href="login.html">← Login</a></div>`;
  }
}
...
const PAGE_TITLES = {
  dashboard: 'Dashboard', hero: 'Hero & Banner', courses: 'Courses',
  teachers: 'Teachers', gallery: 'Gallery', videos: 'Videos',
  notices: 'Notice Board', students: 'Students', attendance: 'Attendance',
  teacherAttendance: 'Teacher Attendance', leaderboard: 'Leaderboard',
  exams: 'Exams & Results', quiz: 'Quiz Management', routines: 'Routines',
  lessonPlans: 'Lesson Plans', finance: 'Fee Collections & Expenses',
  dueNotifications: 'Due Notifications', siteSettings: 'Site Settings',
  stats: 'Homepage Stats', admins: 'Administrators', logs: 'System Logs'
};
...
/* ============================================================
   HELPER — DELETE FIRESTORE DOC ONLY
   (for gallery use deleteGalleryImage instead)
============================================================ */
...
/* ============================================================
   GALLERY SPECIFIC DELETE
   Deletes from BOTH Google Drive (via GAS) AND Firestore,
   then refreshes the grid — no inline onclick, uses event delegation.
============================================================ */
async function deleteGalleryImage(docId, fileId, gridEl) {
  if (!confirm('Delete this image? This cannot be undone.')) return;
  try {
    showToast('Deleting…', 'info');
    // 1. Delete file from Google Drive (via GAS)
    await deleteFileFromDrive(fileId);
    // 2. Delete Firestore document
    await db.collection('galleries').doc(docId).delete();
    showToast('Image deleted!', 'success');
    // 3. Reload grid — pass the grid container element directly (no global needed)
    await _renderGalleryGrid(gridEl);
  } catch(e) {
...
const TABS = {
    ...
    area.innerHTML = `
      <div class="stats-row">
        <div class="stat-card">
          ...
          <div class="num">${students}</div><div class="lbl">Total Students</div>
      ...
      <div class="stats-row" style="margin-bottom:28px;">
        <div class="stat-card">
          ...
          <div class="num">${dues}</div><div class="lbl">Due Notifications</div>
        ...
        <div class="stat-card">
          ...
          <div class="num">৳${totalFees.toLocaleString()}</div><div class="lbl">Total Fee Collected</div>
        ...
        <div class="stat-card" style="cursor:pointer;" onclick="loadTabById('quiz')">
          ...
          <div class="num"><i class="fas fa-arrow-right" style="font-size:1rem;"></i></div><div class="lbl">Quiz Management</div>
        </div>
        <div class="stat-card" style="cursor:pointer;" onclick="loadTabById('lessonPlans')">
          ...
          <div class="num"><i class="fas fa-arrow-right" style="font-size:1rem;"></i></div><div class="lbl">Lesson Plans</div>
        </div>
      </div>
      <div style="display:grid; grid-template-columns:1fr 1fr; gap:18px;">
        ...
        <div class="card">
          ...
          <div style="font-size:0.85rem; display:flex; flex-direction:column; gap:10px;">
            ...
            <div style="display:flex; justify-content:space-between;"><span style="color:var(--muted)">File Storage</span><span class="chip chip-gold">GAS → Google Drive</span></div>
    ...
    area.innerHTML = `
      <div class="card">
        ...
        <div class="form-grid">
          ...
          <div><button class="btn btn-primary" onclick="saveHero()"><i class="fas fa-save"></i> Save Changes</button></div>
        </div>
      </div>`;
  ...
  async courses(area) {
    area.innerHTML = `
      <div class="card" style="margin-bottom:18px;">
        ...
        <div class="form-grid form-grid-2">
          <div class="form-group"><label class="form-label">Course Title</label><input class="form-input" id="cTitle" placeholder="e.g. SSC Preparation"></div>
          <div class="form-group"><label class="form-label">Tag / Category</label><input class="form-input" id="cTag" placeholder="e.g. Board Exam"></div>
          <div class="form-group" style="grid-column:1/-1;"><label class="form-label">Description</label><textarea class="form-input" id="cDesc" placeholder="Course description…"></textarea></div>
          <div class="form-group"><label class="form-label">Icon (Emoji)</label><input class="form-input" id="cIcon" placeholder="📚"></div>
          <div class="form-group" style="align-self:end;"><button class="btn btn-primary" onclick="addCourse()"><i class="fas fa-plus"></i> Add Course</button></div>
        </div>
      </div>
      <div class="card">
    ...
    async function loadCourseList() {
      ...
      snap.forEach(doc => {
        const c = doc.data();
        rows += `<tr>
          <td>${c.icon||'📚'}</td><td><strong>${c.title}</strong></td>
          <td>${c.description||'—'}</td><td><span class="chip chip-gold">${c.tag||'—'}</span></td>
  ...
  async teachers(area) {
    area.innerHTML = `
      <div class="card" style="margin-bottom:18px;">
        ...
        <div class="form-grid form-grid-2">
          <div class="form-group"><label class="form-label">Full Name</label><input class="form-input" id="tName" placeholder="Teacher name"></div>
          <div class="form-group"><label class="form-label">Subject</label><input class="form-input" id="tSubject" placeholder="e.g. Physics"></div>
          <div class="form-group" style="grid-column:1/-1;">
            <label class="form-label">Photo Source</label>
            <div style="display:flex;gap:8px;margin-bottom:8px;">
              <button type="button" class="btn btn-ghost btn-sm" id="tPhotoTabFile" onclick="switchTeacherPhotoTab('file')" style="background:rgba(201,168,76,0.12);color:var(--gold);"><i class="fas fa-upload"></i> Upload File</button>
              <button type="button" class="btn btn-ghost btn-sm" id="tPhotoTabUrl"  onclick="switchTeacherPhotoTab('url')"><i class="fas fa-link"></i> Image URL</button>
            </div>
            <div id="tPhotoFileWrap"><input class="form-input" type="file" id="tPhoto" accept="image/*"></div>
            <div id="tPhotoUrlWrap" style="display:none;"><input class="form-input" id="tPhotoUrl" placeholder="https://example.com/photo.jpg"></div>
          </div>
          <div class="form-group" style="align-self:end;"><button class="btn btn-primary" onclick="addTeacher()"><i class="fas fa-user-plus"></i> Add Teacher</button></div>
        </div>
      </div>
      <div class="card">
    ...
    window.switchTeacherPhotoTab = (tab) => {
      const fw = document.getElementById('tPhotoFileWrap');
      const uw = document.getElementById('tPhotoUrlWrap');
      const fb = document.getElementById('tPhotoTabFile');
      const ub = document.getElementById('tPhotoTabUrl');
      if (tab === 'file') {
    ...
    window.addTeacher = async () => {
      ...
      if (isUrlMode) {
        photo = document.getElementById('tPhotoUrl')?.value.trim() || '';
      } else {
        const file = document.getElementById('tPhoto')?.files[0];
        if (file) {
          try {
            showToast('Uploading photo…', 'info');
            const result = await uploadViaGAS(file, 'Teachers');
            photo = result.url;
      ...
      try {
        ...
        const ui = document.getElementById('tPhotoUrl');
        if (ui) ui.value = '';
        await loadTeacherList();
      } catch(e) { showToast('Error: ' + e.message, 'error'); }
    };
    ...
    async function loadTeacherList() {
      ...
      snap.forEach(doc => {
        ...
        rows += `<tr>
          ...
          <td><strong>${t.name}</strong></td><td>${t.subject||'—'}</td>
  ...
  /* ================================================================
     FIX SUMMARY:
     1. uploadViaGAS() now returns {url, fileId} — both saved to Firestore
     2. loadGalleryGrid uses event delegation (addEventListener) — NO inline onclick
     3. deleteGalleryImage() deletes from Drive first, then Firestore, then re-renders
     4. _renderGalleryGrid() is a pure function — receives grid element as argument
        so it never depends on window globals
  ================================================================ */
  async gallery(area) {
    area.innerHTML = `
      <div class="card" style="margin-bottom:18px;">
        ...
        <div class="form-grid form-grid-2">
          <div class="form-group" style="grid-column:1/-1;">
            ...
            <div style="display:flex;gap:8px;margin-bottom:8px;">
              <button type="button" class="btn btn-ghost btn-sm" id="gTabFile" onclick="switchGalleryTab('file')" style="background:rgba(201,168,76,0.12);color:var(--gold);"><i class="fas fa-upload"></i> Upload File</button>
              <button type="button" class="btn btn-ghost btn-sm" id="gTabUrl"  onclick="switchGalleryTab('url')"><i class="fas fa-link"></i> Image URL</button>
            </div>
            <div id="gFileWrap"><input class="form-input" type="file" id="gFile" accept="image/*"></div>
            <div id="gUrlWrap" style="display:none;"><input class="form-input" id="gUrl" placeholder="https://example.com/image.jpg"></div>
          </div>
          <div class="form-group"><label class="form-label">Caption (optional)</label><input class="form-input" id="gCap" placeholder="Image caption"></div>
          <div style="align-self:end;"><button class="btn btn-primary" id="addGalleryBtn"><i class="fas fa-plus"></i> Add Image</button></div>
        </div>
      </div>
      <div class="card">
    ...
    window.switchGalleryTab = (tab) => {
      const fw = document.getElementById('gFileWrap');
      const uw = document.getElementById('gUrlWrap');
      const fb = document.getElementById('gTabFile');
      const ub = document.getElementById('gTabUrl');
      if (tab === 'file') {
        fw.style.display=''; uw.style.display='none';
        fb.style.cssText='background:rgba(201,168,76,0.12);color:var(--gold);'; ub.style.cssText='';
      } else {
    ...
    // ── Add image button — uses addEventListener, not inline onclick ──
    ...
    document.getElementById('addGalleryBtn').addEventListener('click', async () => {
      ...
      if (isUrlMode) {
        ...
        try {
          const result = await uploadViaGAS(file, 'Gallery');
          url    = result.url;
      ...
      try {
        // ✅ Save BOTH url and fileId — fileId is needed for Drive deletion
        await db.collection('galleries').add({
          url,
          fileId,  // ← KEY FIX: store Drive file ID for later deletion
          caption: document.getElementById('gCap').value.trim(),
          uploadedAt: firebase.firestore.FieldValue.serverTimestamp()
        });
        showToast('Image added!', 'success');
        document.getElementById('gCap').value = '';
        const gu = document.getElementById('gUrl');
        if (gu) gu.value = '';
        // Reload grid — pass element directly
        await _renderGalleryGrid(gridEl);
      } catch(e) { showToast('Error: ' + e.message, 'error'); }
    });
...
/* ============================================================
   GALLERY GRID RENDERER (standalone function, not inside TABS)
   Receives the grid DOM element directly — no window globals needed.
   Uses event delegation: one listener on the grid container.
============================================================ */
async function _renderGalleryGrid(gridEl) {
  if (!gridEl) return;
  gridEl.innerHTML = '<div class="empty-state" style="grid-column:1/-1"><i class="fas fa-spinner fa-spin"></i><p>Loading…</p></div>';

  try {
    const snap = await db.collection('galleries').orderBy('uploadedAt','desc').get();

    if (snap.empty) {
      gridEl.innerHTML = '<div class="empty-state" style="grid-column:1/-1"><i class="fas fa-images"></i><p>No images yet</p></div>';
      return;
    }
    ...
    snap.forEach(doc => {
      ...
      div.style.cssText = 'position:relative;border-radius:10px;overflow:hidden;background:var(--card2);';

      // ✅ Store docId and fileId as data attributes — no inline onclick needed
      div.dataset.docId  = doc.id;
      div.dataset.fileId = g.fileId || ''; // empty string for URL-only entries

      div.innerHTML = `
        <img src="${g.url}"
             style="width:100%;height:130px;object-fit:cover;display:block;"
             loading="lazy"
             onerror="this.src='https://placehold.co/160x130/152036/c9a84c?text=Error'">
        ${g.caption ? `<div style="padding:6px 8px;font-size:0.72rem;color:var(--muted);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">${g.caption}</div>` : ''}
        <button class="gallery-delete-btn"
                style="position:absolute;top:6px;right:6px;background:rgba(239,68,68,0.92);border:none;
                       border-radius:6px;padding:5px 9px;cursor:pointer;color:#fff;font-size:0.78rem;
    ...
    // Removes any old listener first by cloning the node (clean approach)
    const newGrid = gridEl.cloneNode(true);
    ...
    newGrid.addEventListener('click', async (e) => {
      ...
      const fileId = card.dataset.fileId; // may be empty for URL-only images

      await deleteGalleryImage(docId, fileId, newGrid);
    });

  } catch(e) {
    if (gridEl) gridEl.innerHTML = `<div class="empty-state" style="grid-column:1/-1"><p style="color:var(--red)">Error: ${e.message}</p></div>`;
  }
}

/* ============================================================
   REMAINING TABS (videos, notices, students, attendance,
   teacherAttendance, leaderboard, exams, quiz, routines,
   lessonPlans, siteSettings, stats, admins,
   finance, dueNotifications, logs)
...
Object.assign(TABS, {
  ...
  async videos(area) {
    area.innerHTML = `
      <div class="card" style="margin-bottom:18px;">
        ...
        <div class="form-grid form-grid-2">
          <div class="form-group"><label class="form-label">Video Title</label><input class="form-input" id="vTitle" placeholder="Video title"></div>
          <div class="form-group"><label class="form-label">YouTube Embed URL</label><input class="form-input" id="vUrl" placeholder="https://www.youtube.com/embed/..."></div>
          <div><button class="btn btn-primary" onclick="addVideo()"><i class="fas fa-plus"></i> Add Video</button></div>
        </div>
      </div>
      <div class="card">
  ...
  async notices(area) {
    area.innerHTML = `
      <div class="card" style="margin-bottom:18px;">
        ...
        <div class="form-grid">
          <div class="form-group"><label class="form-label">Notice Title</label><input class="form-input" id="nTitle" placeholder="Notice title"></div>
          <div class="form-group"><label class="form-label">Body / Details</label><textarea class="form-input" id="nBody" placeholder="Notice details…" rows="3"></textarea></div>
          <div><button class="btn btn-primary" onclick="addNotice()"><i class="fas fa-bullhorn"></i> Post Notice</button></div>
        </div>
      </div>
      <div class="card">
  ...
  async students(area) {
        ...
        c.innerHTML = `
          <div class="card">
            ...
            <div class="form-grid form-grid-2">
              ...
              <div class="form-group" style="align-self:end;"><button class="btn btn-primary" onclick="createStudent()"><i class="fas fa-user-plus"></i> Create Student</button></div>
            </div>
          </div>`;
      } else { loadStudentList(c); }
    };
    window.createStudent = async () => {
      ...
      try {
        await db.collection('students').doc(id).set({
          ...
          activeStatus: true, createdAt: firebase.firestore.FieldValue.serverTimestamp()
        });
    ...
    async function loadStudentList(c) {
      ...
      snap.forEach(doc => {
        const s = doc.data();
        rows += `<tr>
          <td><strong>${doc.id}</strong></td><td>${s.name||'—'}</td><td>${s.class||'—'}</td>
          <td>${s.phone||'—'}</td>
      ...
      snap.forEach(doc => {
        ...
        html += `<div style="display:flex;align-items:center;justify-content:space-between;padding:10px 14px;background:var(--input-bg);border-radius:8px;border:1px solid var(--border);">
          ...
          <div style="display:flex;gap:8px;">
            <label style="cursor:pointer;display:flex;align-items:center;gap:4px;font-size:0.85rem;"><input type="radio" name="att_${doc.id}" value="present" checked> <span style="color:var(--green)">Present</span></label>
            <label style="cursor:pointer;display:flex;align-items:center;gap:4px;font-size:0.85rem;"><input type="radio" name="att_${doc.id}" value="absent"> <span style="color:var(--red)">Absent</span></label>
            <label style="cursor:pointer;display:flex;align-items:center;gap:4px;font-size:0.85rem;"><input type="radio" name="att_${doc.id}" value="late"> <span style="color:var(--amber)">Late</span></label>
          </div>
        </div>`;
      });
      html += '</div>';
  ...
  async teacherAttendance(area) {
    area.innerHTML = `
      <div class="card" style="margin-bottom:18px;">
        ...
        <div class="form-grid form-grid-2">
          <div class="form-group"><label class="form-label">Teacher Name</label><input class="form-input" id="ta-name" placeholder="Teacher name"></div>
          <div class="form-group"><label class="form-label">Date</label><input class="form-input" type="date" id="ta-date" value="${new Date().toISOString().split('T')[0]}"></div>
          ...
          <div class="form-group"><label class="form-label">Note (optional)</label><input class="form-input" id="ta-note" placeholder="Optional note"></div>
    ...
    async function loadTeacherAttList() {
      ...
      try {
        const snap       = await db.collection('teacherAttendance').orderBy('date','desc').limit(50).get();
        ...
        el.innerHTML = rows
          ? `<div class="table-wrap"><table><thead><tr><th>Teacher</th><th>Date</th><th>Status</th><th>Note</th><th>Action</th></tr></thead><tbody>${rows}</tbody></table></div>`
  ...
  async leaderboard(area) {
    area.innerHTML = `
      <div class="card" style="margin-bottom:18px;">
        ...
        <div class="form-grid form-grid-2">
          ...
          <div><button class="btn btn-primary" onclick="saveLeaderboard()"><i class="fas fa-trophy"></i> Save Score</button></div>
        </div>
      </div>
      <div class="card">
    ...
    async function loadLB() {
      ...
      snap.forEach(doc => {
        const s = doc.data();
        rows += `<tr>
          <td>${medals[rank-1]||rank}</td><td><strong>${s.name||doc.id}</strong></td>
          <td>${s.class||'—'}</td><td><strong style="color:var(--gold)">${s.score||0}</strong></td>
  ...
  async exams(area) {
    area.innerHTML = `
      <div class="card" style="margin-bottom:18px;">
        ...
        <div class="form-grid form-grid-2">
          ...
          <div><button class="btn btn-primary" onclick="createExam()"><i class="fas fa-file-pen"></i> Create Exam</button></div>
        </div>
      </div>
      <div class="card">
    ...
    async function loadExamList() {
      ...
      snap.forEach(doc => {
        const e = doc.data();
        rows += `<tr>
          <td><strong>${e.title}</strong></td><td>${e.subject||'—'}</td>
          <td>${e.date||'—'}</td><td>${e.total||'—'}</td>
  ...
  async quiz(area) {
    ...
    window.createQuiz = async () => {
      ...
      try {
        await db.collection('quizzes').add({
          title, subject,
          class:    document.getElementById('qz-class').value,
          duration: parseInt(document.getElementById('qz-duration').value) || 30,
    ...
    async function loadQuizList() {
      ...
      try {
        ...
        snap.forEach(doc => {
          const q   = doc.data();
          const sc  = q.status==='active'?'chip-green':'chip-red';
          rows += `<tr>
            <td><strong>${q.title}</strong></td><td>${q.subject}</td><td>${q.class||'All'}</td>
            <td>${q.duration||'—'} min</td>
            <td><span class="chip ${sc}">${q.status||'active'}</span></td>
            <td>
  ...
  async routines(area) {
    area.innerHTML = `
      ...
      <div class="card" style="margin-bottom:18px;">
        ...
        <div class="form-grid form-grid-2">
          ...
          <div class="form-group"><label class="form-label">Routine Image URL</label><input class="form-input" id="trt-url" placeholder="https://drive.google.com/..."></div>
    ...
    window.addRoutine = async () => {
      ...
      if (!title || !url) { showToast('Title and URL required', 'error'); return; }
      try {
        await db.collection('routines').add({ title, class: document.getElementById('rt-class').value, imageUrl: url, type: 'student', createdAt: firebase.firestore.FieldValue.serverTimestamp() });
    ...
    window.addLessonPlan = async () => {
      ...
      try {
        await db.collection('lessonPlans').add({
          subject, class: document.getElementById('lp-class').value,
          date, day: document.getElementById('lp-day').value,
          time: document.getElementById('lp-time').value,
          teacher: document.getElementById('lp-teacher').value.trim(),
          topic, notes: document.getElementById('lp-notes').value.trim(),
          createdAt: firebase.firestore.FieldValue.serverTimestamp()
        });
      ...
      try {
        ...
        snap.forEach(doc => {
          ...
          rows += `<tr>
            ...
            <td style="font-weight:600">${l.topic}</td><td>${l.teacher||'—'}</td>
            <td>${l.time||'—'}</td><td style="color:var(--muted);font-size:0.78rem">${l.notes||'—'}</td>
            <td><button class="btn btn-danger btn-sm" onclick="delDoc('lessonPlans','${doc.id}',loadLessonPlans)"><i class="fas fa-trash"></i></button></td>
          </tr>`;
        });
        el.innerHTML = rows
          ? `<div class="table-wrap"><table><thead><tr><th>Date</th><th>Day</th><th>Subject</th><th>Class</th><th>Topic</th><th>Teacher</th><th>Time</th><th>Notes</th><th></th></tr></thead><tbody>${rows}</tbody></table></div>`
      ...
      <div class="card" style="margin-bottom:18px;">
        ...
        <div class="form-grid form-grid-2">
          ...
          <div class="form-group"><label class="form-label">Fee Type</label>
            <select class="form-input" id="ff-type"><option>Monthly Fee</option><option>Admission Fee</option><option>Exam Fee</option><option>Other</option></select></div>
    ...
    window.recordFee = async () => {
      ...
      try {
        await db.collection('fees').add({
          ...
          year:    document.getElementById('ff-year').value,
          amount, date: now.toISOString().split('T')[0],
          time: now.toLocaleTimeString('en-US',{hour:'2-digit',minute:'2-digit'}),
          recordedBy: 'Super Admin', createdAt: firebase.firestore.FieldValue.serverTimestamp()
        });
        showToast(`Fee recorded: ${name}`, 'success');
    ...
    window.recordExpense = async () => {
      ...
      try {
        await db.collection('expenses').add({
          purpose, amount, date: now.toISOString().split('T')[0],
          time: now.toLocaleTimeString('en-US',{hour:'2-digit',minute:'2-digit'}),
          recordedBy: 'Super Admin', createdAt: firebase.firestore.FieldValue.serverTimestamp()
        });
    ...
    area.innerHTML = `
      <div class="card" style="margin-bottom:18px;">
        ...
        <div class="form-grid form-grid-2">
          ...
          <div class="form-group"><label class="form-label">Due Month</label><input class="form-input" id="du-month" placeholder="e.g. January 2025"></div>
          <div class="form-group"><label class="form-label">Note (optional)</label><input class="form-input" id="du-note" placeholder="e.g. Fee not paid"></div>
  ...
  async siteSettings(area) {
    let s = {};
    try { const doc = await db.collection('settings').doc('site').get(); if (doc.exists) s = doc.data(); } catch(e) {}
    area.innerHTML = `
      <div class="card">
  ...
  async stats(area) {
    ...
    try { const doc = await db.collection('settings').doc('stats').get(); if (doc.exists) s={...s,...doc.data()}; } catch(e) {}
    area.innerHTML = `
      <div class="card">
        ...
        <div class="form-grid form-grid-2">
          ...
          <div><button class="btn btn-primary" onclick="saveStats()"><i class="fas fa-save"></i> Save Stats</button></div>
        </div>
      </div>`;
